    ...
    <div class="container-xxl position-relative bg-white d-flex p-0">
        ...
        <div class="sidebar pe-4 pb-3">
            <nav class="navbar bg-light navbar-light">
                ...
                @php
                    $identitas_id = request()->segment(2);
                    $id = request()->segment(2);
                    session(['identitas_id' => $identitas_id]);
                    session(['id' => $id]);
                @endphp
                <div class="navbar-nav w-100">
                    <a href="{{ url('identitas_user/' . Session::get('id')) }}" class="nav-item nav-link"><i class="fas fa-address-card"></i>Data Diri</a>
                </div>
                <div class="navbar-nav w-100">
                    <a href="{{ url('portofolio_user/' . Session::get('identitas_id')) }}" class="nav-item nav-link"><i class="fas fa-file"></i>Portofolio</a>
                </div>
                <div class="navbar-nav w-100">
                    <a href="{{ url('pendidikan_user/' . Session::get('identitas_id')) }}" class="nav-item nav-link"><i class="fas fa-user-graduate"></i>Pendidikan</a>
                </div>
                <div class="navbar-nav w-100">
                    <a href="{{ url('organisasi_user/' . Session::get('identitas_id')) }}" class="nav-item nav-link"><i class="fas fa-layer-group"></i>Organisasi</a>
                </div>
                <div class="navbar-nav w-100">
                    <a href="{{ url('skill_user/' . Session::get('identitas_id')) }}" class="nav-item nav-link"><i class="fas fa-head-side-virus"></i>Skill</a>
                </div>
                <div class="navbar-nav w-100">
                    <a href="{{ url('kontak_user/' . Session::get('identitas_id')) }}" class="nav-item nav-link"><i class="fas fa-address-book"></i>Kontak</a>
                </div>
                <div class="navbar-nav w-100">
                    <a href="{{ url('gallery_user/' . Session::get('identitas_id')) }}" class="nav-item nav-link"><i class="fas fa-images"></i>Galeri</a>
                </div>                
            </nav>
        </div>
        <!-- Sidebar End -->
